  ...
  return `<div class="app-shell">
    <aside class="sidebar" id="sidebar">
      ...
      <nav aria-label="Primary navigation">
        ${groups.map((group) => `<div class="nav-group"><span>${escapeHtml(group)}</span>${ROUTES.filter((route) => route.group === group).map((route) => `<a href="${route.path}" data-link ${activeRoute?.id === route.id ? 'aria-current="page"' : ""}>${escapeHtml(route.label)}</a>`).join("")}</div>`).join("")}
      </nav>
      <div class="sidebar-footer"><span>Stellar testnet</span><strong>Human approval required</strong></div>
    </aside>
    <div class="app-stage">
