...
export function createPage() {
  ...
  return {
    ...
    render({ account, overview, localPasskey }) {
      ...
      return `<section>
        ${pageHeader({ eyebrow: "Soroban Contract Account V1", title: "Smart Wallet", summary: "A passkey owner grants a short-lived Stellar agent session constrained by merchant, asset, amount, budget, and expiry." })}
        ...
        <div class="wallet-layout">
          ...
          <section><div class="section-heading"><div><span class="section-label">Coordinated proof</span><h2>Contract Account settlement</h2></div></div>${evidenceCard("C-account session", coordinated)}<div class="notice pending"><strong>Agent transfer boundary</strong><p>The session secret stays in the supervised local agent. It is never loaded into this browser.</p></div><div class="panel compact-panel"><div class="section-heading"><div><span class="section-label">EVM authority</span><h2>Privy embedded wallet</h2></div>${statusPill("separate")}</div><p>Base and Avalanche identities are managed independently from the Soroban account.</p><a class="primary-button button-link" href="/treasury" data-link>Open Treasury</a></div></section>
        </div>
